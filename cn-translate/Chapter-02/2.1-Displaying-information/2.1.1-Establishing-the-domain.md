...
首先，我们将关注玉米卷配料。在领域中，玉米卷配料是相当简单的对象。每一种都有一个名称和一个类型，这样就可以在视觉上对其进行分类（蛋白质、奶酪、酱汁等）。每一个都有一个 ID，通过这个 ID 可以轻松、明确地引用它。下面的代码定义了所需的域对象。
...
[https://projectlombok.org/](https://projectlombok.org/) 可获取有关如何将 Lombok 插件安装进您的 IDE 的详细信息。
...
现在我们已经定义了 Ingredient 和 Taco，我们还需要定义客户如何指定他们想要订购的玉米卷，以及付款和送货方式信息，这就是 TacoOrder 类：
