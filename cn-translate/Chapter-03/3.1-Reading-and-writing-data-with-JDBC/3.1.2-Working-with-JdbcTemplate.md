...
通过将 spring.datasource.generate-unique-name 属性设置为 false，我们可以看出 Spring 不再为数据库生成随机值。相反，它应该使用 spring.datasource.name 属性的值。在这种情况下，数据库名称将是“tacocloud”。因此，数据库 URL 将是 “"jdbc:h2:mem:tacocloud”，您可以在 JDBC UR L 中指定 H2 控制台连接。
...
当 Spring 创建 JdbcIngredientRepository bean 时，Spring 会向 bean 注入 JdbcTemplate.这是因为该类只有一个构造方法，Spring 会通过构造方法的参数来隐式调用自动装配依赖。如果类里的构造方法不止一个，或者你想让构造方法被显式地自动装配，那么你需要在构造方法上添加@Autowired 注解。
