...
你也许不是想跳过特定数量的项目，而是需要过一段时间再跳过前几个项目。`skip()` 操作的另一种形式（如图 1`0`.11 所示）是生成一个流，该流在从源流发出项之前等待一段指定的时间。
...
你已经看到了 `take()` 操作的一个例子，但是根据 `skip()` 操作，`take()` 可以看作是 `skip()` 的反面。`skip()` 跳过前几个项，`take()` 只发出前几个项（如图 11.12 所示）：
...
给定一个决定一个项是否通过 Flux 的 Predicate，`filter()` 操作允许你根据需要的任何条件有选择地发布。图 11.14 中的弹珠图显示了 `filter()` 的工作原理。
...
也许你需要过滤的是你已经收到的任何项目。`distinct()` 操作（如图 11.15 所示）产生一个只发布源 Flux 中尚未发布的项的 Flux。
...
在处理流经 Flux 的数据的过程中，你可能会发现将数据流分解成比特大小的块是有帮助的。`buffer()` 操作（如图 11.18 所示）可以解决这个问题。
