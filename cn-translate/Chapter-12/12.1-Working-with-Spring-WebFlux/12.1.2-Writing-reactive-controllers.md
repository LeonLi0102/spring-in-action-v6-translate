...
那就更好了！理想情况下，一个响应式 controller 将是一个端到端的响应式栈的顶端，包括 controller、repository、database 以及两者之间的任何 service。这种端到端的响应式栈如图 12.3 所示：

![](../../assets/12.3.png)
**图 12.3 为了最大限度地发挥响应式 web 框架的优势，它应该是完整的端到端响应式堆栈的一部分。**

这样的端到端的栈要求 repository 被写入以返回一个 Flux，而不是一个 Iterable。在下一章中，我们将探讨如何编写响应式 repository，但下面我们将看一看响应式 TacoRepository 可能是什么样子：
...
哇！这就简单多了。然而，更重要的是，通过返回 `Mono<Taco>` 而不是 Taco，可以使 Spring WebFlux 以一种被动的方式处理响应。因此，您的 API 将更好地响应大的负载。
...
正如最初编写的，`postTaco()` 不仅返回一个简单的 Taco 对象，而且还接受一个绑定到请求主体内容的 Taco 对象。这意味着在请求负载完全解析并用于实例化 Taco 对象之前，无法调用 `postTaco()`。同时，`postTaco()` 内部对 repository `save()` 方法的阻塞调用也会导致等待。简言之，请求被阻塞了两次：一次在进入 `postTaco()` 时，另一次在其内部调用 `save()` 时。但是，通过对 `postTaco()` 应用响应式编码，可以使其成为完全无阻塞的请求处理方法：
