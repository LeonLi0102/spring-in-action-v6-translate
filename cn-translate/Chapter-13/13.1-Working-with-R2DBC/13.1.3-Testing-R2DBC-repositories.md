...
如果我们能够将 TacoOrder 作为聚合根，与拥有的所有 Taco 对象一起持久化，那就太好了。那样，如果我们找到一个 TacoOrder，就能得到完整的 Taco 对象（而不仅仅是 ID）。让我们定义一个 Service 级别类，该类位于 OrderRepository 和 TacoRepository 之上，以模拟第 3 章的 OrderRepository 的行为。