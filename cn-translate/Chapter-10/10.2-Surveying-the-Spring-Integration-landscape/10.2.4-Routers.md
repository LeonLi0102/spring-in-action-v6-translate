...
AbstractMessageRouter bean 从 numberChannel 通道接收消息。匿名内部类检查消息负载，如果是偶数，则返回名为 evenChannel 的通道（在路由器 bean 之后定义）。否则，消息负载中的数字为奇数，此时，将返回名为 oddChannel 的通道（也在 bean 声明方法中定义）。
