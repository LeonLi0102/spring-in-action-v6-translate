...
通过 `@ServiceActivator` 注解 bean，将其指定为一个服务激活器，从名为 someChannel 的信道处理消息。至于 MessageHandler 本身，它是通过一个 lambda 实现。虽然这是一个简单的 MessageHandler，但当收到消息时，它会将有效载荷输出到标准输出。

另外，可以声明一个服务激活器，用于在返回一个新的有效载荷之前处理传入的消息。在这种情况下，这个 bean 应该是一个 GenericHandler 而非 MessageHandler：
...
注意，GenericHandler 不仅给出了有效载荷，还包括消息头（即使本示例未使用头信息）。同时也可以通过传递 MessageHandler 或 GenericHandler 去调用在流定义中的 `handler()` 方法，在 Java DSL 配置中使用服务激活器：
...
当使用 GenericHandler 时，lambda 表达式或方法引用会接收消息的有效载荷和报头作为参数。另外，如果选择在流程末尾使用 GenericHandler，必须返回 null，否则会收到"未指定输出通道"的错误。
