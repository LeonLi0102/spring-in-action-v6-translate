...
在程序清单 10.4 中，首先从名为 textInChannel 的通道接收消息，然后传递给一个转换器，将消息负载转为大写。转换后，消息由出站通道适配器处理，该适配器基于 Spring Integration 文件模块提供的文件类型创建。最后，调用 `get()` 构建并返回 IntegrationFlow。简而言之，这个 bean 方法定义了与 XML 和 Java 配置示例等效的集成流。
