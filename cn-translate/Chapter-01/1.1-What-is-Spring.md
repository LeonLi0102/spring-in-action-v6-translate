...
任何有一定规模的应用程序都由许多组件组成，每个组件负责自己那部分功能，并与其他组件协调配合共同完成工作。在运行应用程序时，需要以某种方式创建这些组件并相互引用。
...
将 bean 连接在一起的行为基于一种称为 _依赖注入_（DI）的模式。使用依赖注入的应用程序，组件自身不再负责创建和维护它所依赖的其他 bean，而是由一个单独的实体（容器）来创建和管理所有组件，并将它们注入到需要的地方。通常通过构造函数参数或属性访问器方法完成此操作。
